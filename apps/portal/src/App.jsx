...
export default function Home() {
  return (
    <div className="relative min-h-full flex flex-col items-center overflow-hidden">
      ...
      {/* ── Products Showcase ── */}
      <div id="projects" className="relative z-10 w-full max-w-5xl mx-auto px-6 pb-24 pt-10">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold tracking-tight mb-3">Our Visualization Engines</h2>
          <p className="text-zinc-500">Breaking down complex topics frame by frame.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* VisualDB Card */}
          <div className="panel p-6 bg-zinc-900/40 border border-zinc-800/80 hover:border-zinc-700/80 transition-all group flex flex-col h-full relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4">
              <span className="bg-emerald-400/10 text-emerald-400 text-[10px] font-bold px-2 py-1 rounded-full border border-emerald-400/20">LIVE NOW</span>
            </div>
            ...
            <h3 className="text-2xl font-bold mb-3 text-zinc-100">VisualDB</h3>
            <p className="text-zinc-400 leading-relaxed mb-8 flex-1">
              Master SQL queries by watching them execute step-by-step. Understand JOINs, aggregations, and filtering through animated row-by-row execution traces.
            </p>
            
            <a
              href="http://localhost:5174"
              className="inline-flex items-center justify-center w-full gap-2 px-4 py-3 rounded-md bg-accent/10 hover:bg-accent/20 text-accent font-semibold transition-colors border border-accent/20"
            >
              Launch VisualDB
          ...
          {/* DSA Visual Card */}
          <div className="panel p-6 bg-zinc-950/40 border border-zinc-800/50 flex flex-col h-full relative overflow-hidden">
             <div className="absolute top-0 right-0 p-4">
              <span className="bg-amber-400/10 text-amber-400 text-[10px] font-bold px-2 py-1 rounded-full border border-amber-400/20">COMING SOON</span>
            </div>
            
            <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center mb-6 opacity-60">
              <Code2 className="text-amber-400" size={24} />
            </div>
            
            <h3 className="text-2xl font-bold mb-3 text-zinc-300">DSA Visual</h3>
            <p className="text-zinc-500 leading-relaxed mb-8 flex-1">
              Algorithms you can see. Watch pointers glide, trees recurse, and DP tables fill in. Step through popular LeetCode patterns frame by frame.
            </p>
            
            <button
              disabled
              className="inline-flex items-center justify-center w-full gap-2 px-4 py-3 rounded-md bg-zinc-900/50 text-zinc-600 font-medium border border-zinc-800 cursor-not-allowed"
            >
              In Development
              <Sparkles size={14} />
            </button>
          ...
          {/* System Design Visual Card */}
          <div className="panel p-6 bg-zinc-950/40 border border-zinc-800/50 flex flex-col h-full relative overflow-hidden lg:col-span-2">
             <div className="absolute top-0 right-0 p-4">
              <span className="bg-zinc-500/10 text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-full border border-zinc-500/20">FUTURE ROADMAP</span>
            </div>
            ...
            <h3 className="text-2xl font-bold mb-3 text-zinc-300">System Design Visual</h3>
            <p className="text-zinc-500 leading-relaxed mb-8 flex-1 max-w-3xl">
              Understand large-scale architecture. Watch requests flow through load balancers, hit caches, scale horizontally, and write to distributed databases in real-time.
            </p>
          </div>
        </div>
      </div>
